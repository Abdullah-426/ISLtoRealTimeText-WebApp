...
export function TypedBar() {
    ...
    const processTranscript = useStore((s: any) => s.processTranscript);
    const isProcessing = useStore((s: any) => s.isProcessing);
    ...
    return (
        <div className='bg-black/50 backdrop-blur-sm border border-white/10 p-6 rounded-2xl shadow-sm space-y-3'>
            ...
            <textarea
                className='min-h-[160px] w-full p-3 rounded-xl bg-black/30 border border-white/10 text-white placeholder-gray-500 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent'
                value={transcript}
            ...
            <div className='flex gap-2'>
                <button
                    className={`px-4 py-2 rounded-full shadow transition ${isProcessing
