#!/usr/bin/env python3
"""
Quick test of V5 phrase endpoints
"""
import requests
import json
import numpy as np
...
def test_v5_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ TCN endpoint working")
            print(f"  Top prediction: {result.get('top_label', 'N/A')}")
            print(f"  Confidence: {result.get('top_prob', 0):.3f}")
            print(f"  Commit OK: {result.get('commit_ok', False)}")
        else:
            print(f"✗ TCN endpoint failed: {response.status_code}")
            print(f"  Response: {response.text}")
    except Exception as e:
        print(f"✗ TCN endpoint error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"✓ Ensemble endpoint working")
            print(f"  Top prediction: {result.get('top_label', 'N/A')}")
            print(f"  Confidence: {result.get('top_prob', 0):.3f}")
            print(f"  Commit OK: {result.get('commit_ok', False)}")
        else:
